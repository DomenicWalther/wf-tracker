export interface HonoriaItem {
  id: string;
  name: string;
  location: string;
}

export interface HonoriaSection {
  id: string;
  title: string;
  items: HonoriaItem[];
}
...
export const SECTIONS: HonoriaSection[] = [
  {
    id: 'roathe',
...
// Assign stable IDs once at module load
...
export const TOTAL_HONORIA_ITEMS = SECTIONS.reduce((sum, s) => sum + s.items.length, 0);
